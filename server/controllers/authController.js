...
export const setupPasscode = async (req, res) => {
    ...
    try {
        ...
        const { error } = await supabase.from('users').update({ passcode_hash: hashed }).eq('id', userId);
        if (error) throw error;
...
export const loginWithPasscode = async (req, res) => {
    ...
    try {
        ...
        if (checkOnly) return res.status(200).json({ message: 'User exists' });
        ...
        if (!isValid) {
            ...
            if (failedAttempts >= 5) lockedUntil = new Date(Date.now() + 10 * 60000);
...
export const loginWithEmail = async (req, res) => {
    ...
    try {
        ...
        if (!isValid) {
            let failedAttempts = (user.failed_login_attempts || 0) + 1;
            let lockedUntil = failedAttempts >= 5 ? new Date(Date.now() + 10 * 60000) : null;
            await supabase.from('users').update({ failed_login_attempts: failedAttempts, locked_until: lockedUntil }).eq('id', user.id);
            return res.status(401).json({ error: 'Invalid passcode' });
        }
...
export const oauthStart = (req, res) => {
    ...
    if (!clientId) return res.status(500).send('Google OAuth not configured on server');
    const params = new URLSearchParams({ client_id: clientId, redirect_uri: redirectUri, response_type: 'code', scope: 'openid email profile', access_type: 'offline', prompt: 'consent' });
...
export const resetPasscode = async (req, res) => {
    ...
    try {
        const { data: log, error } = await supabase.from('otp_logs').select('*').eq('user_id', userId).eq('type', 'email').order('created_at', { ascending: false }).limit(1).single();
        if (error || !log || log.otp_code !== otp || new Date(log.expiry_time) < new Date()) {
            return res.status(400).json({ error: 'Invalid or expired OTP' });
        }
        if (!validatePasscodeFormat(newPasscode)) return res.status(400).json({ error: 'Invalid passcode format' });
...
export const verifyOTP = async (req, res) => {
    ...
    try {
        const { data: log, error } = await supabase.from('otp_logs').select('*').eq('type', 'sms').order('created_at', { ascending: false }).limit(1).single();
        if (error || !log || log.otp_code !== otp || new Date(log.expiry_time) < new Date()) {
            return res.status(400).json({ error: 'Invalid or expired OTP' });
        }
        await supabase.from('otp_logs').delete().eq('id', log.id);
